...
# 1 – point to the env that already contains all wheels
use_condaenv("r-reticulate", required = TRUE)

# 2 – optional sanity check (warn, don’t install)
missing <- c("pandas", "numpy", "scipy",
             "matplotlib", "seaborn",
             "statsmodels", "graphviz", "linearmodels")[!
           vapply(c("pandas","numpy","scipy","matplotlib",
                     "seaborn","statsmodels","graphviz","linearmodels"),
                   py_module_available, logical(1))]

if (length(missing))
  warning("Python modules missing: ",
          paste(missing, collapse = ", "),
          ". Install them once into the 'r-reticulate' env and re-render.")
...
| **Weakness** | Clunky syntax; many 'dialects' | Overkill for many, complex development environment | Clunkiest machine learning, no native explainable programing, cost. |
...
<td><p>Execute the following commands in a <strong>terminal</strong>:</p>
<p><code>bash Miniforge3-MacOSX-&lt;arch&gt;.sh</code></p>
<p><code>conda init "$(basename "$SHELL")"</code></p>
<p><code>exec "$SHELL"</code></p>
<p><code>which mamba || conda install -n base -c conda-forge mamba</code></p>
<p>conda activate stats &amp;&amp; python -m ipykernel install --user --name stats --display-name Python (stats)</p></td>
<td><p>This sets up an environment stats with all needed packages, then activates a Kernel (controls the versions and packages that are used).</p></td>
</tr>
<tr>
<td><p>3:</p></td>
<td><p>Install Visual Code</p></td>
<td><p><a href="https://code.visualstudio.com/download">https://code.visualstudio.com/download</a> then add Python and Jupyter extensions. </p>
<p>In VS Code: Command Palette → Shell Command: Install ‘code’ command in PATH</p></td>
